...
// Lazy-load iframe only when drawer is open
const ChatContent = () => (
...
const FloatingChatDrawer = () => {
  ...
  // Auto-trigger chat on scroll
  useEffect(() => {
    const handleScroll = () => {
      if (window.scrollY > 700 && !isOpen) {
        setIsOpen(true);
      }
    };
    window.addEventListener("scroll", handleScroll);
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-24 right-4 w-[90vw] max-w-md z-50 bg-slate-950 rounded-xl shadow-2xl p-4"
          >
            {/* Close Button */}
            <button
